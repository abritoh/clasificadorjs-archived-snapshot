...
angular.module('ngIdle.idle', ['ngIdle.keepalive', 'ngIdle.localStorage'])
  .provider('Idle', function() {
    ...
    this.$get = ['$interval', '$log', '$rootScope', '$rootScope','$document', 'Keepalive', 'IdleLocalStorage', '$window',
      function($interval, $log, $rootScope, $Scope, $document, Keepalive, LocalStorage, $window) {
        var state = {
          idle: null,
          timeout: null,
          idling: false,
        ...
        function toggleState() { 
          ...
          //--[ABH] change to EMIT if event should be buubled up
          $rootScope.$broadcast('Idle' + name);          
          //$Scope.$emit('Idle' + name);
        ...
        if ($window.addEventListener) $window.addEventListener('storage', wrap, false);
            else $window.attachEvent('q', wrap);
